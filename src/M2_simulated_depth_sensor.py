...
def find_object(scene, obj_global) :
    ...
    scene.remove_duplicated_points()
    scene.remove_non_finite_points()
...
def main():
    ...
    y = 0.4
    z = 0.21
    file = open("pose_(x-02 y04).csv",'a+')
    sigma = [0.1,0.03,0.01,0.001,0]
    for j in range(5):
        file.write("Sigma = " + str(sigma[j]) + ";\nx;y;\n")
        for i in range(50):
